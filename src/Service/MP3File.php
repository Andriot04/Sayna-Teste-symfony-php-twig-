...
class MP3File
{
    ...
    public function getDurationEstimate($filename)
    {
        return $this->getDuration($use_cbr_estimate=true,$filename);
    }

    //Read entire file, frame by frame... ie: Variable Bit Rate (VBR)
    public function getDuration($use_cbr_estimate=false,$filename)
    {
        ...
        while (!feof($fd))
        {
            ...
            {
                return $this->estimateDuration($info['Bitrate'],$offset,$filename);
            }
        }
        return round($duration);
    }

    private function estimateDuration($bitrate,$offset,$filename)
    {
        $kbps = ($bitrate*1000)/8;
